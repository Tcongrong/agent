...
public class UserService {
    ...
    public User register(RegisterRequest request) {
        logger.info("开始用户注册流程，用户名: {}, 手机号: {}", request.getUsername(), request.getPhone());
        ...
        if (request.getEmail() == null || !isValidEmail(request.getEmail())) {
            logger.warn("邮箱格式不正确: {}", request.getEmail());
            throw new BusinessException(ErrorCode.INVALID_PARAMETER, "邮箱格式不正确，请输入有效的邮箱地址");
        }
        ...
        try {
            if (userMapper.findByEmail(request.getEmail()) != null) {
                logger.warn("邮箱已存在: {}", request.getEmail());
                throw new BusinessException(ErrorCode.USER_ALREADY_EXISTS, "邮箱已存在");
            }
        } catch (BusinessException be) {
            throw be;
        } catch (Exception e) {
            logger.error("查询邮箱时发生异常: {}", e.getMessage(), e);
            throw new BusinessException(ErrorCode.INTERNAL_ERROR, "系统错误，请稍后再试");
        }
        
        // 检查手机号是否已存在（如果提供）
        if (request.getPhone() != null && !request.getPhone().isEmpty()) {
            logger.info("检查手机号是否已存在: {}", request.getPhone());
            if (isPhoneAlreadyExists(request.getPhone())) {
                logger.warn("手机号已被注册: {}", request.getPhone());
                throw new BusinessException(ErrorCode.USER_ALREADY_EXISTS, "手机号已被注册，请更换其他手机号");
            }
        }
    ...
    public User login(LoginRequest request) {
        ...
        logger.debug("存储的密码: {}", user.getPasswordHash());
    ...
    private boolean isPhoneAlreadyExists(String phone) {
        try {
            ...
            // 如果找到多个记录，也视为手机号已被注册
            logger.warn("手机号已被注册（数据库中存在多条记录）: {}", phone);
            return true;
        } catch (Exception e) {
            logger.error("检查手机号是否存在时发生异常: {}", e.getMessage(), e);
            // 为了安全起见，如果发生异常，我们也假设手机号已存在
            // 这样可以避免在数据库出现问题时创建重复用户
            return true;
        }
    }
